...
export default function LaporanPenjualanProduk() {
  ...
  const fetchData = async () => {
    try {
      ...
      let url = `${API_URL}/laporan/penjualan-produk?`;
  ...
  const tableData = useMemo(
    () =>
      data.map((d, i) => ({
        ...
        nomor_struk: d.nomor_struk || "-",
        nama_produk: d.nama_produk,
        jumlah: d.jumlah,
        harga_satuan: formatRupiah(d.harga_satuan),
        total: formatRupiah(d.total),
        _search: `${d.store} ${d.nomor_struk} ${d.nama_produk}`.toLowerCase(),
      })),
    [data],
  );
  ...
  const handlePrintPDF = () => {
    ...
    // === SUB-HEADER ===
    doc.setFontSize(12);
    doc.text("LAPORAN PENJUALAN PRODUK", pageWidth / 2, 18, {
      align: "center",
    });
    ...
    if (filterType === "Harian") {
      ...
      periodeText = "Semua Data Penjualan Produk";
    }
    ...
    const totalSemua = data.reduce((sum, d) => sum + (d.total || 0), 0);
    ...
    rows.push([
      ...
      {
        content: totalSemua.toLocaleString("id-ID"),
  ...
  const columns = [
    ...
    { key: "nomor_struk", label: "No Faktur" },
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 border-b border-gray-100 pb-4">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Rekap seluruh data penjualan produk.
                </p>
              </div>
