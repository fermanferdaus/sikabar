...
import useFetchPengeluaranDetail from "../../hooks/useFetchPengeluaranDetail";
...
export default function PengeluaranDetail() {
  ...
  const { pengeluaran, storeName, loading, error, refetch } =
    useFetchPengeluaranDetail(id_store);
    ...
    aksi: (
      <div className="flex items-center justify-left gap-2">
        <button
          onClick={() =>
            navigate(
              `/pengeluaran/edit/${p.id_pengeluaran}?id_store=${id_store}`,
            )
          }
        ...
        return (
          <>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 space-y-8 transition-all duration-300">
              {/* Back Button */}
              <BackButton to="/pengeluaran"/>

              {/* === Header === */}
              <div className="flex flex-col sm:flex-row justify-between items-center gap-3 border-b border-gray-100 pb-4">
                <div>
                  <h1 className="text-xl font-semibold text-slate-800">
                    Detail Pengeluaran – {storeName || `Store #${id_store}`}
                  </h1>
                  <p className="text-sm text-gray-500 mt-1">
                    Lihat daftar dan total pengeluaran cabang berdasarkan
                    periode dan kategori.
                  </p>
                </div>

                {/* Tombol Aksi */}
                <div className="order-1 sm:order-2 flex justify-start sm:justify-end w-full sm:w-auto gap-2">
                  <button
                    onClick={() => {
