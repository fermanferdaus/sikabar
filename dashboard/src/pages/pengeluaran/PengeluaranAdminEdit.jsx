...
export default function PengeluaranAdminEdit() {
  const { id_pengeluaran } = useParams();
  const [searchParams] = useSearchParams();
  const id_store = searchParams.get("id_store"); // ✅ Ambil dari query params
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Simpan notifikasi agar muncul di halaman detail
      ...
      // ✅ Arahkan ke halaman yang sesuai (cabang atau global)
      if (id_store) navigate(`/pengeluaran/${id_store}`);
      else navigate("/pengeluaran");
    } catch (err) {
      ...
      <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-10 transition-all duration-300 relative">
        ...
        <div className="border-b border-gray-100 pb-5 mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Ubah data pengeluaran sesuai kebutuhan. Semua kolom bersifat
            opsional.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8 text-left">
          ...
          <div className="flex justify-end gap-4 pt-6 border-t border-gray-100">
            <button
              ...
              onClick={() =>
                id_store
                  ? navigate(`/pengeluaran/detail/${id_store}`)
            ...
            <button
              type="submit"
              className="px-6 py-2.5 rounded-lg font-medium text-white bg-blue-600 hover:bg-blue-700 transition"
            >
              Simpan Perubahan
            </button>
          </div>
