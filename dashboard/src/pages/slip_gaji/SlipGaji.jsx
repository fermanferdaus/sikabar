  ...
  useEffect(() => {
    ...
    const fetchSlip = async () => {
      try {
        ...
        const url =
          role === "kasir"
            ? `${API_URL}/gaji/slip?role=${role}&id_kasir=${idKasir}`
            : `${API_URL}/gaji/slip?role=${role}&id_capster=${idCapster}`;
          ...
          <div className="text-[15px] space-y-1">
            ...
            {data.potongan_bulan_ini > 0 && (
              <>
                ...
                {data.daftar_potongan?.length > 0 && (
                  <div className="mt-2 pl-3 border-l-2 border-gray-200 text-sm text-gray-600 space-y-1">
                    {data.daftar_potongan.map((p, i) => (
                      <div key={i}>
                        • {p.keterangan || "-"} (Rp{" "}
