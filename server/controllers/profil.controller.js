...
export const updateProfil = async (req, res) => {
  try {
    const { nama_barbershop, telepon, instagram, tiktok } = req.body;
    ...
    await db.query(
      `
      UPDATE profil 
      SET nama_barbershop = ?, telepon = ?, instagram = ?, tiktok = ?
      WHERE id_profil = ?
      `,
      [nama_barbershop, telepon, instagram, tiktok, rows[0].id_profil]
    );
