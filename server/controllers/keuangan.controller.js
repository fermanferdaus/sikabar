...
export const getKeuanganSummary = async (req, res) => {
  try {
    ...
    const params = Array(16).fill([bulan, tahun]).flat();
    ...
    const total = rows.reduce(
      (acc, r) => {
        const bersih =
          ...
          Number(r.total_laba_produk) -
          (Number(r.total_komisi_capster) +
            Number(r.total_pengeluaran) +
            Number(r.total_potongan_umum) +
            Number(r.total_potongan_kasbon) +
        ...
        return {
          ...
          pengeluaran:
            acc.pengeluaran + Number(r.total_pengeluaran),
          potongan_umum: acc.potongan_umum + Number(r.total_potongan_umum),
          potongan_kasbon: acc.potongan_kasbon + Number(r.total_potongan_kasbon),
...
export const getKeuanganStoreSummary = async (req, res) => {
  try {
    ...
    const params = [
      id_store, bulan, tahun,   // pendapatan
      id_store, bulan, tahun,   // komisi
      id_store, bulan, tahun,   // laba produk
      id_store, bulan, tahun,   // pengeluaran
      id_store, bulan, tahun,   // bonus capster
      id_store, bulan, tahun,   // bonus kasir
      id_store,                 // gaji capster
      id_store,                 // gaji kasir
      id_store, id_store, bulan, tahun, // potongan umum
      id_store, id_store, bulan, tahun  // potongan kasbon
    ];
